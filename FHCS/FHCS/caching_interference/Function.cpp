...
//将节点坐标写入到本地文档
void Node_write(int num, Node *p)
{
	ofstream file;
	file.open("node_50.txt");
	for(int i=0;i<num;i++)
	{
	    file<<p[i].NodeGetXAxis()<<" ";
		file<<p[i].NodeGetYAxis();
		file<<endl;
	}
	file<<endl;
	file.close();
...
//从本地文档中读取节点坐标
void Node_read(int num, Node *p)
{
	 ifstream file;
	 file.open("F:\\cache_state_3\\node_100.txt"); 
	 double x;
     double y;
	 for(int i=0; i<num; i++)
	 {
...
void Out_NodeNeighborTable(int (& NodeNeighborTable)[MaxRelayNum][MaxUserNum])
{
	ofstream file;
	file.open("用户与基站的邻接关系-100.txt");
	for(int i=0;i<MaxRelayNum;i++)
	{
		for(int j=0;j<MaxUserNum;j++)
		{
			file<<NodeNeighborTable[i][j]<<" ";
		}
		file<<endl;
	}
	file.close();
...
//初始化目标函数：
void Get_Objective_function(int (& Objective_function)[variable_num],int (& NodeNeighborTable)[MaxRelayNum][MaxUserNum],int (& User_Request)[MaxUserNum][MaxFileNum],int (& Mode_Table)[Mode_Num][MaxRelayNum],int Mode_Num_Real,int *User_Mode_Num)
{
	//初始化
...
//得到等式矩阵系数
/*void Get_Matrix_equation(int ( & Matrix_equation)[equation_num][variable_num],int ( & Mode_Table)[Mode_Num][MaxRelayNum],int Mode_Num_Real,int equation_number)
{
	//初始化
	...
	{		
		Matrix_equation[i][i] = 1;
		Matrix_equation[i][i+equation_number] = 1;					
	}
	cout<<"等式的数目为： "<<equation_number<<endl;
}    

//输出等式矩阵
void Print_Matrix_equation(int ( & Matrix_equation)[equation_num][variable_num],int equation_number,int variable_number)
{
...
int Get_Matrix_inequality(int (& Mode_Table)[Mode_Num][MaxRelayNum],int (& Matrix_inequality_value)[inequality_num],int Mode_Num_Real,int *User_Mode_Num,int(& NodeNeighborTable)[MaxRelayNum][MaxUserNum],double *Rate, int(& Matrix_inequality)[inequality_num][variable_num])
{
	//不等式矩阵初始化
  ...
  /* int fore_num2 = Mode_Num_Real;
   ...
   {	 
	...
	Matrix_inequality_value[inequality_number] = Cache_remain[i];
	inequality_number++;
   ...
   for(int i=0;i<MaxUserNum;i++)
   {
	 for(int j=temp3; j<temp3+User_Mode_Num[i];j++)
	 {
		  ...
		  Matrix_inequality[inequality_number][j] = - (int)(Rate[temp3]*1000);	   //将速率乘以1000，mbps转换为kbps
